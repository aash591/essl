...
/**
 * PATCH /api/db/users/batch-designation
 * Batch update user designations
 * Body: { userIds: number[], designationId: number | null }
 */
export async function PATCH(request: NextRequest) {
  try {
    const body = await request.json();
    const { userIds, designationId } = body;
    ...
    console.error("Error batch updating user designations:", error);
    return NextResponse.json<ApiResponse<null>>({
      success: false,
